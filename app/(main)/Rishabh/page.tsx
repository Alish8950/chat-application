...
const Rishabh = () => {
  const { socket, countAlish, messagesRishabh } = useGlobalUser();
  ...
  const joinRoom = () => {
    if(alish !== "") {
      socket.emit("join_room", alish);
    }
  }

  const handleSendMessage = () => {
    socket.emit("send_message_1", {message, alish});
    setMessage("");
  };
  useEffect(() => {
    ...
    <>
      <div className="main flex">
        <div className="left w-fit px-5 py-3 bg-primary h-screen">
          ...
          <div className="chat-component flex flex-col gap-3" onClick={joinRoom}>
        ...
        <div className="right bg-primary w-full px-5 py-3 border-s border-white">
          ...
          <div className="bg-neutral rounded-2xl mt-8 h-[calc(100%-84px)] flex flex-col justify-end">
            {messagesRishabh.map((msg, index) => (
            <div key={index} className="own-message flex items-center text-primary gap-3 px-4">
              <SecondaryAvatar />
